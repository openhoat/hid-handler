...
that = _.extend(new events.EventEmitter(), {
  ...
  handleUsbAttach: function (device) {
    ...
    that.registerHid({type: deviceType, device: device, interface: supportedDevice.interface}).then(function () {
  ...
  registerHid: function (opt, cb) {
    ...
    return p.do(
      ...
      function initHid() {
        ...
        hid.device.type = opt.type;
        hid.device.interfaceIndex = opt.interface;
      },
      function openDevice() {
      ...
      function claimInterface() {
        ...
        hid.device.interfaceIndex = hid.device.interfaceIndex || 0;//hid.device.interfaces.length - 1;
        hid.interface = hid.device.interface(hid.device.interfaceIndex);
        if (!hid.interface) {
  ...
  unregisterHid: function (hid, cb) {
    ...
    return p.do(
      ...
      function releaseInterface() {
        ...
        return p.fromNode(hid.interface.release.bind(hid.interface, true))
          .then(function () {
      ...
      function registerDevices() {
        ...
        return p.each(_.values(supportedDevices),
          function (supportedDevice) {
            ...
            return that.registerHid({type: deviceType, device: device, interface: supportedDevice.interface});
          });
      })
